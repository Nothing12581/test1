...
class OptimalDispatcher:
    ...
    def _add_constraints(self, model: pyo.ConcreteModel):
        ...
        def supply_demand_balance_rule(model, t): #这一时刻的预测总电荷（demand[t]），就是模型传过来的预测结果
        ...
        def ramp_up_constraint_rule(model, g, t): #发电机存在巨大的机械惯性，不能像开关一样瞬间满载
        ...
        def storage_soc_dynamic_rule(model, s, t):
            ...
            if t == 1:#规定时间步，物理上的起点
                ...
                efficiency = self.storage[s].get('efficiency', 0.95)#充放电效率，考虑热能损耗
                dt = 0.25  #时间跨度，0.25小时=15分钟
