...
class RollingOptimizer:
    ...
    def __init__(self, 
                 ...
                 data_config: Dict,
                 window_size: int = 96,#可在此调整滑动窗口大小，目前是96（预测未来24小时）
                 step_size: int = 1):
        ...
        self.beta = data_config.get('feedback_coefficient', 0.5)'''反馈系数，如果是0，即完全信任训练；
                                                                     如果设置的比较大，比如0.8，系统就会对误差极其敏感
                                                                     导致调度剧烈震荡'''
    ...
    def _generate_forecast(self, start_idx: int) -> np.ndarray:
        ...
        feature_cols = ['load', 'temp_avg', 'humidity', 'rain', 'quarter_hour'] #特征可以在这里加，注意对应的训练脚本input_dim相应改动
